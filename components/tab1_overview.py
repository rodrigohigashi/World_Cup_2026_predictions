...
def _hero_html(team: str, prob: float, n: int,
    ...
    return (
        ...
        f'<span style="font-size:.63rem;color:rgba(237,242,255,.32);letter-spacing:.04em">'
...
def _matchup_cards_html(matchups: list) -> str:
    ...
    for m in matchups:
        ...
        cards += (
            ...
            f'<span style="font-size:.68rem;font-weight:800;color:{nc_h};'
            f'font-variant-numeric:tabular-nums">{ph:.0f}%</span>'
            f'<span style="font-size:.6rem;color:{T3}">Emp {pd_:.0f}%</span>'
            f'<span style="font-size:.68rem;font-weight:800;color:{nc_a};'
...
def _ranking_html(prob_campea: pd.Series) -> str:
...
def render(prob_campea: pd.Series, phase_probs: pd.DataFrame,
           ...
           matchups=None, n_historico: int = 0):
    ...
    st.markdown(_ranking_html(prob_campea), unsafe_allow_html=True)
